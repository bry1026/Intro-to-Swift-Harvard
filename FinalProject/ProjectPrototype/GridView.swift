...
@IBDesignable class GridView: UIView {
    ...
    var cols: Int {
        get {
            if let configuration = engine.configuration {
                let maxCol = configuration.positions.map { $0.col }.maxElement() ?? 20
                
                var size: Int = 20 // default is 20 by 20
                
                size = maxCol + 1    // give it a margin of space
                if size > 100 {
    ...
    var rows: Int {
        get {
            if let configuration = engine.configuration {
                let maxRow = configuration.positions.map { $0.row }.maxElement() ?? 20

                
                var size: Int = 20 // default is 20 by 20
                
                size = maxRow + 1    // give it a margin of space
                if size > 100 {
                    size = 100   // we will limit it at 100 by 100
                }
                if size < 20 {
